...
if [ ! "$have_flasher" -gt 0 ] ; then
	...
	options=("Raspberry Pi" "CH341A" "Tigard" "FT232H" "Exit")
...
if [ "${FLASHER}" = "rpi" ] ; then
	...
	programmer="ch341a_spi"
        ...
        echo "Ok. Connect an FT232H programmer"
...
if [ ! "$have_chipname" -gt 0 ] ; then
	...
	if [ ! "$chip_found" -gt 0 ] ; then
		CHIPNAME=$(cat "${TEMP_DIR}"/chips | grep Found | grep "W25Q64JV-.Q" | grep -o '".*"' | grep -oP '"\K[^"\047]+(?=["\047])' || true)
		if [ ! -z "${CHIPNAME}" ] ; then
			chip_found=1
		fi
	fi
