...
if [ "$request_update" -gt 0 ] ; then
	...
	UPSTREAM_X230=$(echo ${UPSTREAM_FILE} | grep x230_fhd)
	if [[ -z "$UPSTREAM_X230" ]] ; then
		echo "The latest release didn't include the X230_fhd"
		exit 0
	fi
	...
	if [[ "$CURRENT_VERSION" = "$UPSTREAM_VERSION" ]] ; then
		echo -e "${GREEN}You are using the latest version of Skulls for the X230_fhd${NC}"
		exit 0
	elif [[ "$CURRENT_VERSION" < "$UPSTREAM_VERSION" ]] ; then
		echo -e "${RED}You have ${CURRENT_VERSION} but there is version ${UPSTREAM_VERSION} available for the X230_fhd. Please update.${NC}"
		...
		case "$response" in
			[yY][eE][sS]|[yY])
				...
				mkdir skulls-x230_fhd-${UPSTREAM_VERSION}
				tar -xf ${UPSTREAM_FILE} -C skulls-x230_fhd-${UPSTREAM_VERSION}/
				echo "Version ${UPSTREAM_VERSION} extracted to ../skulls-x230_fhd-${UPSTREAM_VERSION}/"
		...
		echo "You seem to use a development version. Please use release package skulls-x230_fhd ${UPSTREAM_VERSION} for flashing."
	fi
...
if [ ! "$have_input_image" -gt 0 ] ; then
	image_available=$(ls -1 | grep x230_fhd_coreboot_seabios || true)
	...
	options=( $(find -maxdepth 1 -name "x230_fhd_coreboot_seabios*rom" -print0 | xargs -0) )
...
while true; do
	...
	case $yn in
		[Yy]* ) cd ${OUTPUT_PATH} && ${FLASHROM} --force --noverify-all -p internal:boardmismatch=force,laptop=force_I_want_a_brick --layout ${LAYOUT_FILENAME} --image bios -w "${OUTPUT_IMAGE_NAME}"; break;;
		[Nn]* ) exit;;
		* ) exit;;
	esac
done
